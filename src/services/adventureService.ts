...
interface AdventureAnalytics {
  ...
  favoriteInterests: Array<{ interest: string; count: number }>;
...
class AdventureService {
  async generateAdventure(userId: string, requestData: AdventureRequestData): Promise<IAdventure> {
    try {
      ...
      const adventure = new Adventure({
        ...
        status: 'generated',
        steps: adventureData.steps.map((step: any) => ({
          type: step.type,
          name: step.name,
  ...
  async getAdventure(adventureId: string, userId: string): Promise<IAdventure> {
    try {
  ...
  async getUserAdventures(userId: string, status: string | null = null, limit: number = 20, offset: number = 0): Promise<IAdventure[]> {
    try {
  ...
  async startAdventure(adventureId: string, userId: string): Promise<IAdventure> {
    try {
      const adventure = await Adventure.findOne({
        _id: adventureId,
        userId: userId
      ...
      if ((global as any).io) {
        (global as any).io.to(`user_${userId}`).emit('adventure_started', {
  ...
  async completeAdventure(adventureId: string, userId: string, feedback: AdventureFeedback | null = null): Promise<IAdventure> {
    try {
      const adventure = await Adventure.findOne({
        _id: adventureId,
        userId: userId
      ...
      if ((global as any).io) {
        (global as any).io.to(`user_${userId}`).emit('adventure_completed', {
          adventureId: adventure._id,
          name: adventure.name,
  ...
  async updateAdventureStep(adventureId: string, stepIndex: number, userId: string, action: string): Promise<IAdventure> {
    try {
      const adventure = await Adventure.findOne({
        _id: adventureId,
        userId: userId
      ...
      if ((global as any).io) {
        (global as any).io.to(`adventure_${adventureId}`).emit('step_updated', {
          stepIndex,
          action,
          step: step
        });
  ...
  async getAdventureRecommendations(userId: string, location: { lat: number; lng: number }, limit: number = 5): Promise<IAdventure[]> {
    try {
      const user = await User.findById(userId);
      if (!user) {
        throw new Error('User not found');
  ...
  async shareAdventure(adventureId: string, userId: string, shareData: ShareData): Promise<IAdventure> {
    try {
      const adventure = await Adventure.findOne({
        _id: adventureId,
        userId: userId
      ...
      if (shareData.friends) {
        adventure.social.friendsInvited = shareData.friends.map(id => new mongoose.Types.ObjectId(id));
      }
      ...
      if (shareData.friends && shareData.friends.length > 0) {
        ...
        for (const friendId of shareData.friends) {
          await Notification.createFriendNotification(
            ...
            'friend_invited',
            userId,
            `${user?.name || 'Someone'} invited you to join their adventure: ${adventure.name}`
          );
        }
      }
  ...
  async scheduleAdventureNotifications(adventure: IAdventure): Promise<void> {
    try {
  ...
  validateAdventureRequest(requestData: AdventureRequestData): void {
  ...
  async getAdventureAnalytics(userId: string, timeRange: string = '30d'): Promise<AdventureAnalytics> {
    try {
      const user = await User.findById(userId);
      if (!user) {
        throw new Error('User not found');
  ...
  private calculateAverageRating(adventures: IAdventure[]): number {
  ...
  private getFavoriteInterests(adventures: IAdventure[]): Array<{ interest: string; count: number }> {
  ...
  private calculateCompletionRate(adventures: IAdventure[]): number {
  ...
  private calculateAverageDuration(adventures: IAdventure[]): number {
    if (adventures.length === 0) return 0;
    const totalDuration = adventures.reduce((sum, a) => sum + a.totalDuration, 0);
    return totalDuration / adventures.length;
  }

  private getTimeDistribution(adventures: IAdventure[]): { morning: number; afternoon: number; evening: number; night: number } {
    const distribution = {
      morning: 0,
      afternoon: 0,
      evening: 0,
    ...
    adventures.forEach(adventure => {
      const hour = new Date(adventure.metadata.generatedAt || new Date()).getHours();
